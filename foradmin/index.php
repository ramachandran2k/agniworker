...
$conn=mysqli_connect('localhost','agni_workers','celkonc345','admin');

$aguser="SELECT * FROM `admin log` WHERE username= '$userid' ";
...
if($agname ==$userid && $password== $agmobile ){
	...
	header('location:http://localhost/website/gtcf/foradmin/template/index.php?id='.$agid.'');
  $_SESSION["usar_name"]=$originalname;
	


}else{
	header('location:index.php?id='.$agid.'  ');
